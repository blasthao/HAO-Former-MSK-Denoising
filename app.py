...
def get_model_path(ckpt_name: str) -> str:
    """获取 save 目录下模型权重的绝对路径，兼容打包与云端部署"""
    if hasattr(sys, '_MEIPASS'):
        base_dir = Path(sys._MEIPASS)
    else:
        base_dir = Path(__file__).resolve().parent
    return str(base_dir / "save" / ckpt_name)
...
def run_single_pipeline(input_path, output_path, model, device, batch_size, is_dicom_zip, lang_dict, sub_progress):
    ...
    with torch.no_grad(): 
        ...
        del con_tensor_3d, x_model_input, pred_tensor, out_physical_tensor
...
def main():
    ...
    batch_size = st.sidebar.selectbox(t['batch_size'], [1], index=0)
    ...
    if st.button(t['run_btn'], type="primary", width='stretch'):
        if not uploaded_files:
            ...
            for idx, uploaded_file in enumerate(uploaded_files):
                ...
                if "PD" in filename:
                    model_ckpt = get_model_path("PD_Full_Transformer_best.ckpt")
                    seq_type = "PD"
                elif "T1" in filename:
                    model_ckpt = get_model_path("T1_Full_Transformer_best.ckpt")
                    seq_type = "T1"
                else:
